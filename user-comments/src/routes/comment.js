...
router.post('/', async (req, res) => {
    ...
    */
    ...
    try {
        ...
        console.error(err);
        return res.status(500).json({ message: "Error while verifing user existence!" });
    }
    ...
    if (!user) {
        try {
            ...
            console.error(err);
            return res.status(500).json({ message: "Error while inserting a new user!" });
        }
    }
    ...
    try {
        ...
        console.error(err);
        return res.status(500).json({ message: "Error while inserting a new comment!" });
    }
})
